...
// Problem: Huffman Decoding (For hackerRank submission)
...
static void decode(String S ,Node root){
    if(S == null || S.length() == 0 || root == null) return;
    ...
    for(int i = 0; i < S.length(); i++){
        ...
        if( curNode.data == '\0'){ // internal node
            continue;
        } else { 
...
// Optimization point: 
// We can reduce space complexity without using the 'StringBuilder' if we don't need to return the result. 
// We can just use 'System.out.print(ch)' in the else statement. 
...
* Problem Statement
  ...
  You need to print the actual string.
...
* Explanation
  ...
  S[0]='1' : we move to the right child of the root. We encounter a leaf node with value 'A'. 
             We add 'A' to the decoded string. We move back to the root.

  S[1]='0' : we move to the left child. 
  S[2]='0' : we move to the left child. We encounter a leaf node with value 'B'. 
             We add 'B' to the decoded string. We move back to the root.
  ...
  S[5]='1' : we move to the right child. We encounter a leaf node with value C'. 
             We add 'C' to the decoded string. We move back to the root.
